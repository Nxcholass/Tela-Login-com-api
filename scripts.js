function procurarcep (){ //função que irá requisitar informações, receber json e colocar a informação no campo certo
    let cep = document.getElementById('txtcep').value; //irá pegar o cep digitado no input do cep
    if (cep !== ""){ //se o campo do cep for diferente de vazio, irá realizar a busca
        ...
        req.onload = function(){ //irá tratar a resposta da requisição, pegando os dados recebidos via json e colocando nos campos corretos
            if(req.status === 200){
                ...
                document.getElementById("txtlogradouro").value = procurarendereco.street;
                document.getElementById("txtbairro").value = procurarendereco.neighborhood; 
                document.getElementById("txtcidade").value = procurarendereco.city; 
                document.getElementById("txtestado").value = procurarendereco.state;                
            }
